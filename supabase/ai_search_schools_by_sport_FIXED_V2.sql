-- ai_search_schools_by_sport.sql - FIXED V2 (resolves ambiguous column reference)
...
BEGIN
  ...
  sport_achievements AS (
    ...
    SELECT
      ssr.code::integer as school_code,
      ARRAY_AGG(DISTINCT
        CASE
  ...
  other_sports AS (
    ...
    SELECT
      sss2.code as school_code,
      ARRAY_AGG(sss2.sport ORDER BY sss2.score DESC)
        FILTER (WHERE sss2.score >= 60 AND LOWER(sss2.sport) != LOWER(sport_name)) AS strong_sports
    FROM school_sports_scores sss2
    WHERE sss2.year = 2024
    GROUP BY sss2.code
  ),
  cop_latest AS (
  ...
  LEFT JOIN sport_data sd ON sd.school_code = s.code
  LEFT JOIN sport_achievements sa ON sa.school_code = s.code
  LEFT JOIN other_sports os ON os.school_code = s.code
  WHERE
    -- Gender filter
    (
      gender_pref = 'Any'
